...
public final class GtinUtils {
    ...
    public static Result toGtin14(String input) {
        String digits = normalizedNumeric(input);
        ...
        if (digits.length() == 7 || digits.length() == 11) {
            ...
            return new Result(leftPad(completed, 14), "Missing check digit calculated");
        }
    ...
    private static String normalizedNumeric(String value) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            else throw new IllegalArgumentException("GTIN conversion supports numeric barcodes only");
        }
        return out.toString();
    }
